...
MODEL_NAME=learned_64_orig_super

python -u models/train.py \
  --model_encode_steps=16 \
  --model_decode_steps=1000 \
  ...
  --train_epochs=1.0 \
  --train_log_every=10 \
  --decoding_warmup_steps=5000 \
  --decoding_warmup_stages=50 \
  --resume_checkpoint \
  --mp_skip_nonfinite \
  --no_train \
  --do_predict \
  ...
  --gin_param="trajectory_from_step.set_checkpoint = True" \
