...
MODEL_NAME=learned_64_transformer_fc_p
...
python -u models/train.py \
  ...
  --gin_param="my_forward_tower_factory.num_transformer_layers = 2" \
  --gin_param="Transformer.num_heads = 2" \
  --gin_param="DenseBlock.widening_factor = 2" \
  --gin_param="my_forward_tower_factory.dropout_rate = 0.1" \
  --gin_param="my_forward_tower_factory.output_scale_fn = @my_fixed_scale" \
  --gin_param="my_fixed_scale.rescaled_one = 0.01" \
